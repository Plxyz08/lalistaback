...
const httpComentario = {
    ...
    getComentarios: async (req, res) => {
        try {
            const comentarios = await Comentario.find().populate('idUser', '_id nombre image');
    ...
    getComentarioById: async (req, res) => {
        try {
            const { id } = req.params;
            const comentario = await Comentario.findById(id).populate('idUser', '_id nombre image');
            if (!comentario) {
                return res.status(400).json({ error: helpersGeneral.errores.noEncontrado });
            }
    ...
    getComentariosByIdUser: async (req, res) => {
        try {
            ...
            const comentarios = await Comentario.find({ idUser: idUser }).populate('idUser', '_id nombre image');;
            if (!comentarios || comentarios.length === 0) {
                return res.status(400).json({ error: helpersGeneral.errores.noEncontrado });
            }
            const comentariosConFechaFormateada = comentarios.map((comentario) => {
    ...
    postAddComentario: async (req, res) => {
        try {
            ...
            const userPropietario = await Publicacion.findById(idPublicacion);
            const nuevaNotificacion = new Notificacion({
                idUser: userPropietario.idUser,
